...
- **WRDS / CRSP**  
  ...
  - The data cleaning included only keeping true dividend payment columns as well as relevant ratios.  
  - A new column was created to classify dividend changes as an increase, decrease, or no change based on the next quarter’s dividend.  
  - Each company was assigned an industry label based on the ticker.  
  - Missing values across key financial ratios were filled using ticker-specific averages.  
...
- **yfinance**  
  Used for live testing data. Financial ratios are calculated manually by combining line items from income statements, balance sheets, and cash flow statements.
